...
public class HandOnMain {
	public static void main(String[] args) {
		// 金融資産の初期化
		List<FinancialAsset> assetList = initAssetList();

		// リスクリターンと相関行列のデータ
		FinancialDataSource dataSource = new MyFinancialDataSource();
		// FinancialDataSource dataSource = new SampleFinancialDataSource();
		...
		// 乱数生成クラス
		RandomGenerator randomGenerator = new MyRandomGenerator();
		// シミュレーション実行クラス
		MonteCarloSimulation simulation = new MyMonteCarloSimulation(randomGenerator);
		// シミュレーションの実行
		List<SimulationResult> results = simulation.runSimulation(portfolio, numberOfSimulations, numberOfYears);

		// 結果を出力
		SimulationResultPrinter printer = new MySimulationResultPrinter();
		...
		// 結果をファイルに保存
		SimulationResultExporter exporter = new MySimulationResultExporter(new File("export"));
